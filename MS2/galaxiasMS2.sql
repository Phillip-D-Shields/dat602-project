...
CREATE TABLE tblVectorAsset (
    ...
    FOREIGN KEY (ElementID)
        REFERENCES tblElemen (ElementID)
);
...
elements insert into
    tblElemen (ElementName, ElementDescription, ElementValue)
values
    ('gold', 'shiny shiny', '10'),
...
create procedure pilotLogin(
    in pPilotName varchar(30),
    in pPilotPassword varchar(20)
) begin declare verifyPilot int default null;

if exists (
    select
        PilotID -- PilotName is existing
    from
        tblPilot
    where
        PilotName = pPilotName
...
where
    PilotName = pPilotName
    and PilotPassword = pPilotPassword into verifyPilot;
...
select
    'pilot locked - message admin' as message;
...
select
    'success - pilot online' as message;
...
create procedure pilotLogout(in pPilotID int) begin
update
    tblPilot
set
    PilotOnline = false
...
select
    'success - pilot offline' as message;
...
select
    'bad name or password' as message;
...
select
    'success - pilot deleted' as message;
...
if exists (
    ...
    from
        tblGalaxy
    where
        GalaxyName = pGalaxyName
) then
select
    'bad galaxy name' as message;

else 
insert into
    tblGalaxy(GalaxyName, maxPilots, ActivePilots, GalaxyStatus)
values
    (pGalaxyName, maxPilots, ActivePilots, GalaxyStatus);
...
-- map instance
insert into
    tblMap(GalaxyID, xMax, yMax)
values
(newGalaxyID, rowMax, colMax);

set
    newMapID = last_insert_id();

-- map tiles
vectorRow: loop vectorCol: loop
insert into
    tblVector(MapID, xPosition, yPosition, VectorActive)
values
    (newMapID, rowMin, colMin, true);
...
if rowMin > 10 then leave vectorRow;
...
getElement: loop addElement: loop
set
    elementVector = (
        select
            floor(rand() *(endVectorID - homeVectorID) + homeVectorID)
    );
...
if not exists (
    ...
    from
        tblVectorAsset
    where
        VectorID = elementVector
) then
set
    elementCount = elementCount + 1;

insert into
    tblVectorAsset(VectorID, ElementID)
values
    (assetTile, elementType);
...
while elementType <= 4 do
insert into
    tblInventory(ElementID, PilotID, SessionID, Quantity)
values
    (elementType, pPilotID, newSessionID, 0);
...
select
    'new galaxy initiated' as message;
...
select
    'you live again' as message;

else -- link to reenterShift procedure with user input
select
    'inaccessible - choose another vecot' as message;
...
select
    'galaxy entered' as message;

else
select
    'galaxy inaccesible' as message;
...
create procedure pilotMove(
    ...
    pxPosition int,
    pyPosition int
) begin declare currentMapID int default null;
...
set
    Score = Score + (
        ...
        from
            tblElemen as e
            join tblVectorAsset as t on e.ElementID = t.ElementID
        where
            t.VectorID = targetVectorID
    )
where
    SessionID = pSessionID;
...
select
    'acceptable shift' as message;

else
select
    'vector inaccessible' as message;

end if;

else
select
    'unacceptable shift' as message;

end if;

else
select
    'cooldown in progress, wait ...' as message;
...
set
    Score = Score + (
        ...
        from
            tblElemen as a
            join tblVectorAsset as t on a.ElementID = t.ElementID
        where
            t.VectorID = targetVectorID
    )
where
...
create procedure adminModify(
    ...
    pAdministrator bool,
    pTotalScore int
) begin declare modifyPilot int default null;
...
select
    'failed - bad pilot id' as message;
...
select
    'success - pilot modified' as message;
...
create procedure adminDelete(pPilotID int) begin declare deletePilot int default null;

select
    PilotID into deletePilot
from
    tblPilot
where
    PilotID = pPilotID;
...
select
    'success - pilot delted' as message;
...
create procedure adminKill(pGalaxyID int) begin if exists (
    select
        GalaxyID
    from
        tblGalaxy
...
select
    'galaxy destroyed' as message;

else
select
    'unacceptable galaxy' as message;
...
delimiter // 
create procedure checkGalaxy(pPilotID int, pGalaxyID int, pMapID int) begin declare galaxyScore int default null;
...
select
    'dead - you belong to the galaxy now' as message;

end if;


if galaxyEmperor is not null
or elementCount <= 0 then -- get winner if null and no more assets on map
set
    galaxyEmperor = (
        select
            PilotID
...
set
    u.TotalScore = u.TotalScore + s.Score
where
    s.GalaxyID = pGalaxyID
    and s.Score > 0;
...
create procedure gettAllPilots() begin
select
    PilotName
from
    tblPilot;
...
create procedure getHighScores() begin
select
    PilotName,
    TotalScore
from
...
create procedure getPilotIneventory(pSessionID int) begin
select
    a.ElementDescription as Item,
    i.Quantity
from
    tblInventory as i
    join tblElemen as a on i.ElementID = a.ElementID
where
    SessionID = pSessionID;
